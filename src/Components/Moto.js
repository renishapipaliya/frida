import React from "react";

function Moto() {
  return (
    <>
      <div className="border border-gray-300 mt-4">
        <img
          src="/assets/Moto.png"
          className="lg:ml-[650px] md:ml-[350px] ml-[120px] lg:h-[55px] md:h-[60px] h-[40px]"
        />
      </div>

      <div className="border border-gray-200 mt-4 overflow-auto ">
        <div className="flex justify-between gap-3 mt-2 w-full h-full overflow-scroll scroll whitespace-nowrap scroll-smooth">
          <div className="border border-black rounded-[5px] ">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/s moto g54.png "
                className="h-[100px] w-[80px] mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]   text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                moto G54 5G (128GB)
              </h1>
              <p className="text-[13px] ml-2">Smooth performance</p>
              <h1 className="font-bold ml-2 text-[20px]">Just ₹12,999*</h1>
            </div>
          </div>

          <div className="border border-black rounded-[5px]">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/motorola edge.png"
                className="h-[100px] w-[80px]  mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]  text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                motorola edge 40 neo
              </h1>
              <p className="text-[13px] ml-2">144Hz 10-bit Curved Screen</p>
              <h1 className="font-bold ml-2 text-[20px]">From ₹21,999*</h1>
            </div>
          </div>

          <div className="border border-black rounded-[5px]">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/g32.png"
                className="h-[100px] w-[80px]  mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]  text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                moto g32
              </h1>
              <p className="text-[13px] ml-2">8GB RAM| 128GB Storage</p>
              <h1 className="font-bold ml-2 text-[20px] ">Just ₹8,999*</h1>
            </div>
          </div>
          <div className="border border-black rounded-[5px]">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/motorola g54.png"
                className="h-[100px] w-[80px]  mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]  text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                moto G54 5G (256GB)
              </h1>
              <p className="text-[13px] ml-2">Smooth performance</p>
              <h1 className="font-bold ml-2 text-[20px] ">₹14,999*</h1>
            </div>
          </div>
          <div className="border border-black rounded-[5px]">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/moto e13.png"
                className="h-[100px] w-[80px]  mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]  text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                moto e13
              </h1>
              <p className="text-[13px] ml-2">Big Battery|Premium Design</p>
              <h1 className="font-bold ml-2 text-[20px] ">Just ₹7,499</h1>
            </div>
          </div>
          <div className="border border-black rounded-[5px]">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/motorola edge.png"
                className="h-[100px] w-[80px]  mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]  text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                motorola edge 40 neo
              </h1>
              <p className="text-[13px] ml-2">144Hz 10-bit Curved Screen</p>
              <h1 className="font-bold ml-2 text-[20px] ">From ₹21,999*</h1>
            </div>
          </div>
          <div className="border border-black rounded-[5px]">
            <div className="bg-gray-300 rounded-t-[5px]">
              <img
                src="assets/g32.png"
                className="h-[100px] w-[80px]  mx-auto"
              ></img>
              <div className="bg-blue-600 ml-[3px] mr-[3px] ">
                <h1 className="lg:text-[12px] md:text-[16px] text-[12px]text-center text-white">
                  *incl.₹1,0000 off on Exch
                </h1>
              </div>
            </div>
            <div>
              <h1 className="font-bold lg:text-[14px] md:text-[17px] ml-2">
                moto g32
              </h1>
              <p className="text-[13px] ml-2">8GB RAM| 128GB Storage</p>
              <h1 className="font-bold ml-2 text-[20px] ">Just ₹8,999*</h1>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default Moto;
